...
@SpringBootTest(classes = SerdiClientApplication.class,
	webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@ActiveProfiles("test")
public class SerdiClientApplicationTests {
	...
	public void shouldRegisterClientInEurekaServer() throws InterruptedException {
		...
		ResponseEntity<String> response = this.testRestTemplate.getForEntity(
			"http://localhost:" + this.port + "/service-instances/serdi-client", String.class);

		then(response.getStatusCode()).isEqualTo(HttpStatus.OK);
		then(response.getBody()).contains("serdi-client");
	}
